...
public class EnableClassChange : MonoBehaviour
{
    public GameObject TrocaDeClasse; // Reference to the UI_Shop GameObject
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            TrocaDeClasse.SetActive(true); // Enable the UI_Shop GameObject
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player")) // Check if the collider belongs to the player
        {
            TrocaDeClasse.SetActive(false); // Disable the UI_Shop GameObject
