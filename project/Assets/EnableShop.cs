...
{
    public GameObject UI_Shop; // Reference to the UI_Shop GameObject
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            UI_Shop.SetActive(true); // Enable the UI_Shop GameObject
            try
            {
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player")) // Check if the collider belongs to the player
        {
            UI_Shop.SetActive(false); // Disable the UI_Shop GameObject
